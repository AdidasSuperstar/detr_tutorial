...
from datasets.face import build
...
def get_args_parser():
    ...
    parser.add_argument('--annotations_path', default='',
                        help='path where the annotations are, with prettified bounding boxes x-y-w-h')
...
@torch.no_grad()
def infer(images_path, model, postprocessors, device, output_path, annotations_path):
    model.eval()
    duration = 0
    for img_sample in images_path:
        ...
        result = pd.read_csv(args.annotations_path)
...
if __name__ == "__main__":
    ...
    infer(image_paths, model, postprocessors, device, args.output_dir, args.annotations_path)
